...
window.onclick = function(event) {
  if (!event.target.matches('.filterdropbtn')) {
    ...
    var filterdropdowns = document.getElementsByClassName("filterdropdown-content");
    var i;
    for (i = 0; i < filterdropdowns.length; i++) {
      var openFilterDropdown = filterdropdowns[i];
      if (openFilterDropdown.classList.contains('show')) {
